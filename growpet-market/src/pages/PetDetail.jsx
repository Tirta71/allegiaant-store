...
function PetDetail() {
  ...
  const availableMutations = useMemo(() => {
    ...
    return [...new Set(pet.variants.map((variant) => variant.mutation).filter(Boolean))]
  }, [pet])
  ...
  useEffect(() => {
    ...
    async function loadPet() {
      ...
      try {
        ...
        if (isMounted) {
          setError(requestError.status === 404 ? 'Pet tidak ditemukan.' : requestError.message)
        }
      } finally {
        if (isMounted) {
          setIsLoading(false)
  ...
  useEffect(() => {
    ...
    if (!availableWeights.includes(Number(selectedWeight))) {
      setSelectedWeight(availableWeights[0])
    }
  }, [availableWeights, selectedWeight])

  function handleMutationChange(mutation) {
    const nextWeight = pet?.variants.find((variant) => variant.mutation === mutation)?.weightKg
